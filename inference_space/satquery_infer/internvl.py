...
# Space change: overridable via the MAX_NEW_TOKENS Space variable (bck default is 128).
...
class InternVLAdapter:
    ...
    def __init__(self, model_id: str = DEFAULT_MODEL_ID, device: str | None = None) -> None:
        ...
        # Space change: bf16 on cuda, fp32 on CPU, decided from the device alone (bck also
        # probes torch.cuda.is_bf16_supported(); the Space spec is bf16 on any cuda device).
        self._dtype = torch.bfloat16 if str(self.device).startswith("cuda") else torch.float32
